...
if response.status_code == 200:
        ...
        if st.button("Submit & Process"):
            with st.spinner("Processing..."):
                ...
                if st.session_state.raw_text:
                    ...
                    print(f"Candidate name is :- {get_name(st.session_state.raw_text)}")
    ...
    if ans := st.chat_input(placeholder="Answer: ", disabled=asked, key="node", on_submit=update_value, args=(ans,)) or st.session_state.raw_text:
    ...
    async def send_audio(uri):
        async with websockets.connect(uri) as websocket:
            ...
            while st.session_state.running:
                ...
                st.session_state.transcription = st.session_state.transcription + transcription
    ...
    if st.session_state.transcription:
        ...
        print(f"Transcription send is:- {st.session_state.transcription}")
        if response.status_code == 200:
            st.session_state.messages.append({"role": "ai", "content": response.text})
            st.session_state.question_asked = True                  ####################
            with st.chat_message("ai"):
    ...
    # Display buttons at the bottom of the chat
    display_buttons()
