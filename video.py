...
def display_gif(is_asking):
    if is_asking:
        gif_placeholder.image("L_video.gif", use_column_width=True)
    else:
        gif_placeholder.image("B_video.gif", use_column_width=True)
...
if response.status_code == 200:
    ...
    if ans := st.chat_input(placeholder="Answer: ", disabled=asked, key="node") or st.session_state.raw_text:
        ...
        generate_and_play_audio(response.text, "sample_voice.wav", language="en", file_path="outputs/ai_audio.wav")
