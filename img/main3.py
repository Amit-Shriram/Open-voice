...
template = """
You are an AI interviewer conducting a structured 10-question interview based on the candidate's resume and the job description provided. Your task is to ask questions in a clear, concise manner without any additional explanations or summaries between questions. Each question should be directly related to the information in the resume or job description. 
...
Follow this structure for the 10 questions:

First Greet the candidate by name and ask them to introduce themselves briefly.
after receiving introduction from candidate, ask next 3 questions about the projects mentioned in the candidate's resume. Focus on their role, challenges faced, and outcomes achieved.
then ask next 3 technical questions directly related to the skills and requirements mentioned in the job description.
then ask 2 scenario-based questions relevant to the role and industry mentioned in the job description.
lastly just say Thank you.

Guidelines:
- Do not include lengthy portions of the resume or job description in your questions.
- Focus on one topic per question.
- Give follow-up questions taking the candidate's answers into consideration.
- Output only the next question to be asked, nothing else.

Begin the interview with question 1 and proceed sequentially through all 10 questions.
"""
...
def get_messages_info(messages: list):
    ...
    print(f"#################################################### these are last messages:- {last_messages}")
    value = [SystemMessage(content=formatted_template)] + last_messages
    # print(f"{value}")
    return value

chain = get_messages_info | model      # output of the get_messages_info is input to model, that is what | does
...
async def process_jd(request:JDRequest):
    ...
    return JSONResponse(content={"jd_data":jd_data_global})

@app.post("/start_interview")
async def start_interview():
    # This endpoint will trigger the first AI question
    initial_answer = AnswerModel(ans="", msg_num=0, msg_info=[], uid="initial")
    return await main(initial_answer)
